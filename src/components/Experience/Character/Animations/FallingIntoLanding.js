...
export default class FallingIntoLanding {
  ...
  init() {
    const fallingIntoLandingAnim = this.resources.items.fallingIntoLanding;

    if (
      fallingIntoLandingAnim &&
      fallingIntoLandingAnim.animations &&
      fallingIntoLandingAnim.animations.length > 0
    ) {
      const mixer = this.animationController.mixer;

      if (mixer) {
        this.action = mixer.clipAction(fallingIntoLandingAnim.animations[0]);
  ...
  play(options = {}) {
    ...
    this.animationController.playAnimation("fallingIntoLanding", {
      fadeInDuration,
